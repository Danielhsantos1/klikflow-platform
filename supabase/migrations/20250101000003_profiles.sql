-- profiles: 1:1 mirror of auth.users, holding the app-facing user data.
-- Not tenant-scoped by itself — a user can belong to more than one tenant
...
-- operation at all; RLS policies above still decide *which rows*. Both
...
-- auth.users cascade, never directly by client code.

-- Keeps `profiles` in sync with `auth.users` the moment an account is
-- created, so the rest of the schema can always assume a matching row
